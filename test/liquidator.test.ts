...
contract('Liquidator', accounts => {
  ...
  before('set up contracts', async () => {
    ...
    lendingPool = await LendingPool.at(
      '0x580D4Fdc4BF8f9b5ae2fb9225D584fED4AD5375c'
    );
    oToken = await OptionsContract.at(
      '0x8794fe9332930f7B42aD33cf14b2EFaF7aC49F76'
    );
    optionsExchange = await OptionsExchange.at(
      '0x3B967C6b89458a590bd3948Bd0053E80455D7b0C'
    );
  });
  ...
  describe('#liquidate()', async () => {
    it('test the bytes return values', async () => {
      ...
      const vaultAddressBytes = web3.utils.hexToBytes(
        web3.utils.toHex(creatorAddress)
      );
    ...
    xit('get ETH in AAVE', async () => {
      console.log(
        (await lendingPool.getReserveData(ETHReserveAddress))[1].toString()
      );
    });
